...
async function scrapePage(pageNumber) {
  try {
    ...
    // Find all video items using the actual class structure
    $('.aiovg-item-video').each((index, element) => {
      const $el = $(element);
      
      // Extract video information using the actual HTML structure
      const title = $el.find('.aiovg-title').text().trim();
      const category = $el.find('.aiovg-item-category').text().trim();
      const views = $el.find('.aiovg-views').text().trim().replace('views', '').trim();
      const duration = $el.find('.aiovg-duration').text().trim();
      
      // Extract thumbnail - the site uses YouTube thumbnails
      const thumbnail = $el.find('img').first().attr('src');
      const thumbnailUrl = thumbnail || null;
      
      // Extract tags
      const tags = [];
      $el.find('.aiovg-item-tag').each((i, tag) => {
        const tagText = $(tag).text().trim();
        if (tagText) tags.push(tagText);
      });
      
      // Extract description if available
      const description = $el.find('.aiovg-caption').text().trim();
      ...
      if (title) {
        videos.push({
          ...
          videoUrl: null, // We'll need to extract this from individual video pages
